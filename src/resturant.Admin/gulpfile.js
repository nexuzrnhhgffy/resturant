...
gulp.task('dark', function() {
    return gulp.src('wwwroot/assets/css/dark-style.scss')
        .pipe(sourcemaps.init())
        .pipe(sass())
        .pipe(beautify.css({ indent_size: 4 }))
        .pipe(sourcemaps.write('.'))
...
gulp.task('transparent', function() {
    return gulp.src('wwwroot/assets/css/transparent-style.scss')
        .pipe(sourcemaps.init())
        .pipe(sass())
        .pipe(beautify.css({ indent_size: 4 }))
        .pipe(sourcemaps.write('.'))
...
gulp.task('color', function() {
    return gulp.src('wwwroot/assets/colors/**/*.scss')
        .pipe(sourcemaps.init())
        .pipe(sass())
        .pipe(beautify.css({ indent_size: 4 }))
        .pipe(sourcemaps.write('.'))
        .pipe(gulp.dest('wwwroot/assets/colors'));
});